...
let processGameMessageMap = {
    ...
    newPetalContainer: (data, me, advanced) => {
        if ((window.selfId !== null && room.flowers[window.selfId] !== undefined) || window.spectating == false) {
            ...
            collectedMenu.addPetalContainer(new PetalContainer(new Array(data.petalAmount).fill(new Petal(data.petal)), {x: data.x, y: data.y, w: data.w, h: data.h, originalX: data.originalX, originalY: data.originalY, radius: data.radius, toOscillate: false}, data.id, data.amount ?? 1))
        }
    },
